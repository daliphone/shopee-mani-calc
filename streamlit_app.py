...
with st.sidebar:
    st.header("⚙️ 系統資訊")
    st.markdown('<div style="font-size:11px; color:#95a5a6;">馬尼專用蝦皮計算機<br>版本：V17.5 (全資料庫版)<br>© 2025 Mani Shopee Calc</div>', unsafe_allow_html=True)

# 4. 【完整 PDF 資料庫】 格式: [蝦拍%, 蝦商%]
FEE_DB = {
    "手機平板與周邊": {
        "手機": [5.5, 3.8], "平板電腦": [5.5, 4.0], "穿戴裝置": [5.5, 4.5],
        "對講機": [6.5, 9.5], "手機平板周邊配件": [7.5, 9.5], "其他": [7.5, 9.5]
    },
    "影音": {
        "電視機": [5.5, 5.0], "耳機/耳麥": [5.5, 6.5], "音響/喇叭/麥克風": [6.0, 7.5],
        "擴大機/混音器": [4.0, 6.0], "mp3/mp4播放器": [6.0, 8.0], "影音周邊配件": [6.0, 8.0]
    },
    "家用電器": {
        "大型家電 (冰箱/洗衣機/電視)": [5.3, 5.8], "廚房家電 (氣炸鍋/微波爐)": [5.5, 6.0],
        "生活家電 (吸塵器/電風扇)": [5.5, 6.0], "投影機與周邊": [7.5, 8.5],
        "美容家電 (吹風機/刮鬍刀)": [5.5, 6.5], "居安零件/遙控器/電池": [6.0, 8.0]
    },
    "電腦與周邊配件": {
        "筆記型電腦": [5.0, 4.0], "桌上型電腦": [5.5, 5.0], "螢幕裝置": [5.5, 5.5],
        "電腦零組件": [6.0, 6.5], "儲存裝置 (硬碟/隨身碟)": [5.5, 5.5],
        "網路設備": [6.5, 8.0], "鍵盤滑鼠/印表機": [6.0, 7.0], "電腦周邊/軟體": [7.5, 9.5]
    },
    "電玩遊戲": {
        "遊戲主機": [4.0, 4.0], "遊戲控制器/周邊": [5.5, 6.5], "遊戲片/點數卡": [5.5, 6.5]
    },
    "相機&空拍機": {
        "數位相機/單眼": [5.0, 5.0], "空拍機/運動攝影機": [5.0, 5.0], 
        "鏡頭": [5.0, 5.0], "相機周邊配件": [6.5, 8.0], "望遠鏡": [6.5, 8.5]
    }
}
...
tf1 = round(p * (custom_p_rate / 100)); cf1 = round(p * (p_v_rate / 100)); total_fee1 = tf1 + cf1 + shared_fee; payout1 = p - total_fee1
tf2 = round(p * (custom_s_rate / 100)); cf2 = round(p * (s_v_rate / 100)); total_fee2 = tf2 + cf2 + shared_fee; payout2 = p - total_fee2
f_m_val = cfg_直_前毛_手機 if ("手機" in s_cat_name or "平板" in s_cat_name) else cfg_直_前毛_其他
tf3 = round(p * (f_m_val / 100)); tb3 = round(p * (cfg_直_後毛 / 100)); total_fee3 = tf3 + tb3; payout3 = p - total_fee3
...
with col_拍:
    st.markdown(f"""<div class="result-card"><h3 class="title-拍">蝦拍({p_v_name}{p_v_rate}%)</h3>
        ...
        <div class="total-fee-tag">手續費總計: -${total_fee1:,.0f}</div><hr>
...
with col_商:
    st.markdown(f"""<div class="result-card"><h3 class="title-商">蝦商({s_v_name}{s_v_rate}%)</h3>
        ...
        <p class="expense-tag">金流/活動費: -${shared_fee:,.0f}</p>
        <div class="total-fee-tag">手續費總計: -${total_fee2:,.0f}</div><hr>
...
with col_直:
    st.markdown(f"""<div class="result-card"><h3 class="title-直">蝦皮直送</h3>
        ...
        <p style="color:#95a5a6; font-size:0.85em; margin: 20px 0;">(不計金流/活動/券)</p><hr>
