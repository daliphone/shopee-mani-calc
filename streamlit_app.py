...
st.markdown("""
    ...
    .result-card { border: 1px solid #e6e9ef; padding: 20px; border-radius: 12px; background-color: #ffffff; box-shadow: 0 4px 10px rgba(0,0,0,0.05); min-height: 600px; }
...
with st.sidebar:
    st.header("⚙️ 系統資訊")
    st.markdown('<div style="font-size:11px; color:#95a5a6;">馬尼專用蝦皮計算機<br>版本：V18.5 (PDF完全對齊版)<br>© 2025 Mani Shopee Calc</div>', unsafe_allow_html=True)

# 4. 【三層完整 PDF 資料庫】 
# 結構說明：首頁 > 第二層 > 第三層 (若無則顯示為"無細分"): [蝦拍%, 蝦商%]
DB_3L = {
    "手機平板與周邊": {
        "手機與平板電腦": {
        ...
        "手機平板周邊配件": {
            "保護殼/保護貼/掛繩": [7.5, 9.5], "行動電源/充電傳輸線": [7.5, 9.5], "手機鏡頭/自拍桿": [7.5, 9.5], "對講機": [6.5, 9.5]
        }
    },
    "影音": {
        "電視機與周邊配件": {
            "電視機": [5.5, 5.0], "影音周邊配件": [6.0, 8.0], "電視盒/影音播放器": [6.0, 8.0]
        },
        "音響與耳機": {
            "耳機/耳麥": [5.5, 6.5], "喇叭/藍牙音箱": [6.0, 7.5], "麥克風": [6.0, 7.5], "擴大機/混音器": [4.0, 6.0]
        }
    },
    "家用電器": {
        "廚房家電": {
            "大型廚房家電(冰箱/微波爐)": [5.3, 5.8], "飲水設備": [5.5, 6.5], "烘焙家電/烤箱": [5.5, 6.0]
        },
        "生活家電": {
            "吸塵器/掃地機器人": [5.5, 6.0], "電風扇/冷氣": [5.5, 6.0], "美容家電 (吹風機)": [5.5, 6.5]
        },
        "其他家電": {
            "投影機": [7.5, 8.5], "電池/延長線/遙控器": [6.0, 8.0]
        }
    },
    "電腦與周邊配件": {
        "筆記型電腦": { "(無細分)": [5.0, 4.0] },
        "電腦主機與螢幕": {
            "桌上型電腦": [5.5, 5.0], "螢幕裝置": [5.5, 5.5]
        },
        "儲存裝置": {
            "硬碟 (HDD/SSD)": [5.5, 5.5], "記憶卡/隨身碟": [5.5, 5.5]
        },
        "電腦零組件": {
            "顯示卡": [6.0, 6.5], "主機板/CPU/記憶體": [6.0, 6.5]
        },
        "網路設備與周邊": {
            "網路通訊設備": [6.5, 8.0], "鍵盤/滑鼠": [6.0, 7.0], "電腦軟體": [7.5, 9.5]
        }
    },
    "電玩遊戲": {
        "遊戲主機與配件": {
            "遊戲主機": [4.0, 4.0], "遊戲控制器": [5.5, 6.5], "周邊配件": [5.5, 6.5]
        },
        "遊戲片與點數卡": {
            "遊戲片": [5.5, 6.5], "虛擬點數卡": [5.5, 6.5]
        }
    },
    "相機&空拍機": {
        "單眼與數位相機": {
            "單眼/類單眼相機": [5.0, 5.0], "數位相機": [5.0, 5.0], "鏡頭": [5.0, 5.0]
        },
        "空拍機與攝影": {
            "空拍機": [5.0, 5.0], "運動/數位攝影機": [5.0, 5.0]
        },
        "相機周邊": {
            "相機包/腳架/濾鏡": [6.5, 8.0], "望遠鏡": [6.5, 8.5]
        }
    }
}
...
with col_in:
    ...
    st.markdown("---")
    # 三層連動選單
    l1 = st.selectbox("1. 首頁分類", list(DB_3L.keys()))
    l2 = st.selectbox("2. 第二層分類", list(DB_3L[l1].keys()))
    l3_dict = DB_3L[l1][l2]
    l3_name = st.selectbox("3. 第三層分類", list(l3_dict.keys()))
    
    # 取得初始費率
    rates_init = l3_dict[l3_name]
    s_cat_display = f"{l2} > {l3_name}" if l3_name != "(無細分)" else l2
    
    with st.expander("⚙️ 全局參數與公式設定", expanded=False):
        custom_p_rate = st.number_input(f"【{s_cat_display}】蝦拍率 (%)", value=rates_init[0], step=0.1)
        custom_s_rate = st.number_input(f"【{s_cat_display}】蝦商率 (%)", value=rates_init[1], step=0.1)
        st.markdown("---")
        col_v1_1, col_v1_2 = st.columns([2, 1])
        v1_rate = col_v1_1.number_input("10倍券回饋 (%)", value=3.0, step=0.1)
        v1_target = col_v1_2.selectbox("配置到", ["蝦拍", "蝦商"], key="v1_t")
        col_v2_1, col_v2_2 = st.columns([2, 1])
        v2_rate = col_v2_1.number_input("5倍券回饋 (%)", value=1.5, step=0.1)
...
with col_拍:
    st.markdown(f"""<div class="result-card"><h3 class="title-拍">蝦拍({p_v_n}{p_v_rate}%)</h3>
        <p style="color:gray; font-size:0.85em;">{l1}<br>{s_cat_display}</p><hr>
        <p class="expense-tag">成交手續費: -${tf1:,.0f}</p>
        <p class="expense-tag">{p_v_n}費: -${cf1:,.0f}</p>
...
with col_商:
    st.markdown(f"""<div class="result-card"><h3 class="title-商">蝦商({s_v_n}{s_v_rate}%)</h3>
        ...
        <p class="expense-tag">{s_v_n}費: -${cf2:,.0f}</p>
        <p class="expense-tag">金流/活動費: -${shared_fee:,.0f}</p>
...
with col_直:
    st.markdown(f"""<div class="result-card"><h3 class="title-直">蝦皮直送</h3>
        ...
        <p class="expense-tag">前毛手續費({f_m}%): -${tf3:,.0f}</p>
        <p class="expense-tag">後毛手續費({cfg_直_後毛}%): -${tb3:,.0f}</p>
        <div class="total-fee-tag">手續費總計: -${tot3:,.0f}</div>
        <p style="color:#95a5a6; font-size:0.8em; margin: 20px 0;">(不計金流/活動/券)</p><hr>
...
rows = []
for cl1, sub2 in DB_3L.items():
    for cl2, items3 in sub2.items():
        for cl3, rates in items3.items():
            # 判斷是否為目前選中的第三層品項，是的話套用自定義參數
            pr = custom_p_rate if (cl3 == l3_name and cl2 == l2) else rates[0]
            sr = custom_s_rate if (cl3 == l3_name and cl2 == l2) else rates[1]
            ...
            disp_name = f"{cl1} > {cl2}" if cl3 == "(無細分)" else f"{cl1} > {cl2} > {cl3}"
            rows.append({"PDF完整路徑": disp_name, "蝦拍利潤": int(p_p), "蝦商利潤": int(s_p), "直送利潤": int(d_p)})
