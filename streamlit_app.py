...
with st.sidebar:
    st.header("⚙️ 系統資訊")
    st.markdown('<div style="font-size:11px; color:#95a5a6;">馬尼專用蝦皮計算機<br>版本：V19.0 (PDF 絕對版)<br>© 2025 Mani Shopee Calc</div>', unsafe_allow_html=True)

# 4. 【PDF 規格資料庫】 結構：首頁分類 > 第二層分類 > 第三層分類 (若無則跳過): [蝦拍%, 蝦商%]
# 文字完全咬死 PDF 原文
DB_PDF = {
    "手機平板與周邊": {
        "手機與平板電腦": {
        ...
        "手機平板周邊配件": {
            "保護殼、保護貼與掛繩": [7.5, 9.5], "行動電源與充電傳輸線": [7.5, 9.5], "手機鏡頭與自拍桿": [7.5, 9.5], "對講機": [6.5, 9.5]
        }
    },
    "影音/相機": {
        "電視機與周邊配件": {
            "電視機": [5.5, 5.0], "影音周邊配件": [6.0, 8.0], "電視盒與影音播放器": [6.0, 8.0]
        },
        "相機與空拍機": {
            "單眼與數位相機": [5.0, 5.0], "空拍機與運動攝影機": [5.0, 5.0], "鏡頭": [5.0, 5.0], "相機周邊配件": [6.5, 8.0]
        },
        "音響與耳機": {
            "耳機、耳麥": [5.5, 6.5], "喇叭、藍牙音箱": [6.0, 7.5], "麥克風": [6.0, 7.5], "擴大機與混音器": [4.0, 6.0]
        }
    },
    "家用電器": {
        "廚房家電": {
            "大型廚房家電 (冰箱/微波爐)": [5.3, 5.8], "飲水設備": [5.5, 6.5], "烘焙家電與烤箱": [5.5, 6.0]
        },
        "生活家電": {
            "吸塵器與掃地機器人": [5.5, 6.0], "電風扇與空調": [5.5, 6.0], "美容家電 (吹風機)": [5.5, 6.5]
        }
    },
    "電腦與周邊配件": {
        "筆記型電腦": { "筆記型電腦": [5.0, 4.0] },
        ...
        "儲存裝置": {
            "硬碟 (HDD/SSD)": [5.5, 5.5], "記憶卡與隨身碟": [5.5, 5.5]
        }
    },
    "電玩遊戲": {
        "遊戲主機與配件": {
            "遊戲主機": [4.0, 4.0], "遊戲控制器": [5.5, 6.5], "電玩周邊配件": [5.5, 6.5]
        },
        "遊戲軟體與點數卡": {
...
with col_in:
    ...
    st.markdown("---")
    l1 = st.selectbox("1. 首頁分類", list(DB_PDF.keys()))
    l2 = st.selectbox("2. 第二層分類", list(DB_PDF[l1].keys()))
    l3_list = list(DB_PDF[l1][l2].items())
    l3_item = st.selectbox("3. 第三層分類", l3_list, format_func=lambda x: f"{x[0]} [拍:{x[1][0]}% / 商:{x[1][1]}%]")
    
    s_cat_name = l3_item[0]
    p_rate_db, s_rate_db = l3_item[1]

    with st.expander("⚙️ 全局參數與公式設定", expanded=False):
        custom_p_rate = st.number_input(f"【{s_cat_name}】蝦拍率 (%)", value=p_rate_db, step=0.1)
        custom_s_rate = st.number_input(f"【{s_cat_name}】蝦商率 (%)", value=s_rate_db, step=0.1)
        st.markdown("---")
        v1_rate = st.number_input("10倍蝦拍券回饋 (%)", value=3.0, step=0.1)
        v1_target = st.selectbox("10倍券配置到", ["蝦拍", "蝦商"], key="v1_t")
        v2_rate = st.number_input("5倍蝦拍券回饋 (%)", value=1.5, step=0.1)
        v2_target = st.selectbox("5倍券配置到", ["蝦拍", "蝦商"], index=1, key="v2_t")
        st.markdown("---")
...
with col_直:
    st.markdown(f"""<div class="result-card"><h3 class="title-直">蝦皮直送</h3>
        ...
        <p class="formula-text">公式: {p} × {f_m}%</p>
...
for cl1, sub2 in DB_PDF.items():
    for cl2, items3 in sub2.items():
        for cl3, rates in items3.items():
            ...
            rows.append({"PDF完整路徑": f"{cl1} > {cl2} > {cl3}", "蝦拍利潤": int(p_p), "蝦商利潤": int(s_p), "直送利潤": int(d_p)})
